...
| `DATABASE_ADMIN_DSN` | DSN owner/migration direct atau Supabase Session Pooler khusus command migration/reset; tidak diberikan kepada proses web |
...
Keduanya harus menuju target database yang sama, tanpa daftar multi-host/fallback, serta `sslmode=verify-full` pada staging/production. Guard migration/reset menolak pooler admin generik yang dikenali (`-pooler` atau `pgbouncer`) dan secara khusus mendukung Supabase **Session Pooler port 5432**. Transaction Pooler Supabase port 6543 ditolak. Untuk Neon, penanda hostname `-pooler` dinormalisasi; untuk Supabase, direct/session endpoint dicocokkan berdasarkan project reference, database, dan runtime parameter. Provider lain tetap memakai endpoint direct. Neon juga [merekomendasikan koneksi direct untuk migration](https://neon.com/docs/connect/connection-pooling).
